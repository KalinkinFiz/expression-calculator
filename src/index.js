  ...
  for (let i = 0; i < exprArr.length; i++) {
    ...
    if (exprArr[i] === ")") {
      ...
      while (operator !== "(") {
        ...
        operator = stackOperators.pop();
        if (stackOperators.length === 0 && operator !== "(")
          throw Error("ExpressionError: Brackets must be paired");
      }
      continue;
    }
...
function isValidBrackets(expr) {
  const [count1, count2] = expr.reduce(
    (acc, curr) => {
...
function exprToArray(expr, operators, brackets) {
  ...
  for (let i = 0; i < clearExpr.length; i++) {
    if (Number.isInteger(+clearExpr[i])) {
      number += clearExpr[i];
    } else if (
      operators.includes(clearExpr[i]) ||
      brackets.includes(clearExpr[i])
    ) {
